...
void* packet_receiver(void* arg) {
    for (int i = 1; i <= MAX_ITEMS; i++) {
        ...
        printf("[Receiver Thread] Received Packet ID: %d\n", i * 10);
        
        pthread_cond_signal(&cond); // Processor thread ko alert karo
...
void* packet_processor(void* arg) {
    for (int i = 0; i < MAX_ITEMS; i++) {
        ...
        while (count == 0) {
            pthread_cond_wait(&cond, &lock); // Wait for incoming signal
        }
