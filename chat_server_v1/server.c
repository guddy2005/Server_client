...
void *client_handler(void *arg)
{
	...
	while (1)
	{
		...
		if (find_client(packet.sender) != NULL)
		{
			Packet reply;
			...
			reply.type = REGISTER;
			strcpy(reply.message, "DUPLICATE");
			...
			case CHAT_STATUS:
				{
					...
					if(strcmp(packet.message, "offline") == 0)
					{
						...
						if (self->online == 1) {
							Packet alert_packet;
							strcpy(alert_packet.message, "You are already online.");
							send(self->socket, &alert_packet, sizeof(Packet), 0);
							printf("%s tried to log online but was already logged in\n", self->name);
						} 
						else {
							self->online = 1; 
