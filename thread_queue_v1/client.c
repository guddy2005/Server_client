...
int main() {
    ...
    while (1) {
        ...
        if (strcmp(custom_message, "exit") == 0)
            break;
        ...
        for (int i = 1; i <= total_messages; i++) {
            /*Append \n at the end of each message to enforce delimiter framing */
            snprintf(buffer, BUFFER_SIZE, "%s [Count:%d]\n", custom_message, i);

            if (send_all(client_fd, buffer, strlen(buffer)) < 0) {
                printf("Server disconnected while sending.\n");
                close(client_fd);
                client_fd = connect_to_server(&server_addr);
                break;
            }
            ...
            if (bytes <= 0) {
                printf("Server disconnected while receiving.\n");
                close(client_fd);
                client_fd = connect_to_server(&server_addr);
                break;
            }
